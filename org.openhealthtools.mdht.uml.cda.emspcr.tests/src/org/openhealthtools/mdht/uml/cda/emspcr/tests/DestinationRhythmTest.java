...
public void testValidateDestinationRhythmValueP() {
			OperationsTestCase<DestinationRhythm> validateDestinationRhythmValuePTestCase = new OperationsTestCase<DestinationRhythm>(
			"validateDestinationRhythmValueP",
			operationsForOCL.getOCLValue("VALIDATE_DESTINATION_RHYTHM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return DestinationRhythmOperations.validateDestinationRhythmValueP(
					(DestinationRhythm) objectToTest, diagnostician, map);
			}

		};

		validateDestinationRhythmValuePTestCase.doValidationTest();
}
