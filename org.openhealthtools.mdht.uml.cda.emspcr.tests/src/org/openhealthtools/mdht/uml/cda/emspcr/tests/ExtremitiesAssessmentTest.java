...
public void testValidateExtremitiesAssessmentValueP() {
			OperationsTestCase<ExtremitiesAssessment> validateExtremitiesAssessmentValuePTestCase = new OperationsTestCase<ExtremitiesAssessment>(
			"validateExtremitiesAssessmentValueP",
			operationsForOCL.getOCLValue("VALIDATE_EXTREMITIES_ASSESSMENT_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return ExtremitiesAssessmentOperations.validateExtremitiesAssessmentValueP(
					(ExtremitiesAssessment) objectToTest, diagnostician, map);
			}

		};

		validateExtremitiesAssessmentValuePTestCase.doValidationTest();
}
