...
public void testValidateEMSSceneSectionCodeP() {
			OperationsTestCase<EMSSceneSection> validateEMSSceneSectionCodePTestCase = new OperationsTestCase<EMSSceneSection>(
			"validateEMSSceneSectionCodeP",
			operationsForOCL.getOCLValue("VALIDATE_EMS_SCENE_SECTION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return EMSSceneSectionOperations.validateEMSSceneSectionCodeP(
					(EMSSceneSection) objectToTest, diagnostician, map);
			}

		};

		validateEMSSceneSectionCodePTestCase.doValidationTest();
}
