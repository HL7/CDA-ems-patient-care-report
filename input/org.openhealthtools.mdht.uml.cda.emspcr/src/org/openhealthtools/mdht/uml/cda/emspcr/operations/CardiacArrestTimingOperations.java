...
public class CardiacArrestTimingOperations extends ClinicalStatementOperations {
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.templateId->exists(id : datatypes::II | id.root = '2.16.840.1.1133883.17.3.10.1.123' and id.extension = '2022-01-01')";
	...
	protected static ThreadLocal<  Constraint> VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
    ...
    DatatypesUtil.increment(context,"CardiacArrestTimingCardiacArrestTimingTemplateId","ERROR");
    
  	  
  	  
		if (VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.CARDIAC_ARREST_TIMING);
			try {
				VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_CARDIAC_ARREST_TIMING_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(cardiacArrestTiming)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	protected static ThreadLocal<  Constraint> VALIDATE_CARDIAC_ARREST_TIMING_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
    ...
    DatatypesUtil.increment(context,"CardiacArrestTimingCardiacArrestTimingMoodCode","ERROR");
    
  	  
  	  
		if (VALIDATE_CARDIAC_ARREST_TIMING_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.CARDIAC_ARREST_TIMING);
			try {
				VALIDATE_CARDIAC_ARREST_TIMING_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_CARDIAC_ARREST_TIMING_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_CARDIAC_ARREST_TIMING_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(cardiacArrestTiming)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.code = '67502-5' and value.codeSystem = '2.16.840.1.113883.6.1')";
	...
	protected static ThreadLocal<  Constraint> VALIDATE_CARDIAC_ARREST_TIMING_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
    ...
    DatatypesUtil.increment(context,"CardiacArrestTimingCardiacArrestTimingCode","ERROR");
    
  	  
  	  
		if (VALIDATE_CARDIAC_ARREST_TIMING_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.CARDIAC_ARREST_TIMING);
			try {
				VALIDATE_CARDIAC_ARREST_TIMING_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_CARDIAC_ARREST_TIMING_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_CARDIAC_ARREST_TIMING_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(cardiacArrestTiming)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.codeSystem = '2.16.840.1.113883.6.1' and (value.code = 'LA17054-0' or value.code = 'LA17055-7')))";
	...
	protected static ThreadLocal<  Constraint> VALIDATE_CARDIAC_ARREST_TIMING_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
    ...
    DatatypesUtil.increment(context,"CardiacArrestTimingCardiacArrestTimingValue","ERROR");
    
  	  
  	  
		if (VALIDATE_CARDIAC_ARREST_TIMING_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.CARDIAC_ARREST_TIMING);
			try {
				VALIDATE_CARDIAC_ARREST_TIMING_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_CARDIAC_ARREST_TIMING_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_CARDIAC_ARREST_TIMING_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(cardiacArrestTiming)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
