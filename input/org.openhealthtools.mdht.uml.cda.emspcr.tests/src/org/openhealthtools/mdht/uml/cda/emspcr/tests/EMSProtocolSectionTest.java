...
public void testValidateEMSProtocolSectionCodeP() {
			OperationsTestCase<EMSProtocolSection> validateEMSProtocolSectionCodePTestCase = new OperationsTestCase<EMSProtocolSection>(
			"validateEMSProtocolSectionCodeP",
			operationsForOCL.getOCLValue("VALIDATE_EMS_PROTOCOL_SECTION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return EMSProtocolSectionOperations.validateEMSProtocolSectionCodeP(
					(EMSProtocolSection) objectToTest, diagnostician, map);
			}

		};

		validateEMSProtocolSectionCodePTestCase.doValidationTest();
}
...
public void testGetProtocolObservations() {


EMSProtocolSection target = objectFactory.create();
target.getProtocolObservations();
