...
public void testValidateEMSBillingSectionCodeP() {
			OperationsTestCase<EMSBillingSection> validateEMSBillingSectionCodePTestCase = new OperationsTestCase<EMSBillingSection>(
			"validateEMSBillingSectionCodeP",
			operationsForOCL.getOCLValue("VALIDATE_EMS_BILLING_SECTION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return EMSBillingSectionOperations.validateEMSBillingSectionCodeP(
					(EMSBillingSection) objectToTest, diagnostician, map);
			}

		};

		validateEMSBillingSectionCodePTestCase.doValidationTest();
}
