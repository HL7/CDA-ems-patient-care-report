...
public void testValidateEMSDispatchSectionCodeP() {
			OperationsTestCase<EMSDispatchSection> validateEMSDispatchSectionCodePTestCase = new OperationsTestCase<EMSDispatchSection>(
			"validateEMSDispatchSectionCodeP",
			operationsForOCL.getOCLValue("VALIDATE_EMS_DISPATCH_SECTION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return EMSDispatchSectionOperations.validateEMSDispatchSectionCodeP(
					(EMSDispatchSection) objectToTest, diagnostician, map);
			}

		};

		validateEMSDispatchSectionCodePTestCase.doValidationTest();
}
