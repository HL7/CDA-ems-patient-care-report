...
public void testValidateDrugUseIndicationValueP() {
			OperationsTestCase<DrugUseIndication> validateDrugUseIndicationValuePTestCase = new OperationsTestCase<DrugUseIndication>(
			"validateDrugUseIndicationValueP",
			operationsForOCL.getOCLValue("VALIDATE_DRUG_USE_INDICATION_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return DrugUseIndicationOperations.validateDrugUseIndicationValueP(
					(DrugUseIndication) objectToTest, diagnostician, map);
			}

		};

		validateDrugUseIndicationValuePTestCase.doValidationTest();
}
