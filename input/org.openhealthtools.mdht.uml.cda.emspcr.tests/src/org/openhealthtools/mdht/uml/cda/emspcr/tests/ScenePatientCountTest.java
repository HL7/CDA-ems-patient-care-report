...
public void testValidateScenePatientCountValueP() {
			OperationsTestCase<ScenePatientCount> validateScenePatientCountValuePTestCase = new OperationsTestCase<ScenePatientCount>(
			"validateScenePatientCountValueP",
			operationsForOCL.getOCLValue("VALIDATE_SCENE_PATIENT_COUNT_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return ScenePatientCountOperations.validateScenePatientCountValueP(
					(ScenePatientCount) objectToTest, diagnostician, map);
			}

		};

		validateScenePatientCountValuePTestCase.doValidationTest();
}
