...
public void testValidateCardiacArrestCauseValueP() {
			OperationsTestCase<CardiacArrestCause> validateCardiacArrestCauseValuePTestCase = new OperationsTestCase<CardiacArrestCause>(
			"validateCardiacArrestCauseValueP",
			operationsForOCL.getOCLValue("VALIDATE_CARDIAC_ARREST_CAUSE_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP")
			,objectFactory) {
			...
			protected boolean validate(EObject objectToTest, BasicDiagnostic diagnostician, Map<Object, Object> map) {
				...
				return CardiacArrestCauseOperations.validateCardiacArrestCauseValueP(
					(CardiacArrestCause) objectToTest, diagnostician, map);
			}

		};

		validateCardiacArrestCauseValuePTestCase.doValidationTest();
}
